...
namespace RenPy.Parser
{
	public class RenPyScanner
	{
		...
		public string PeekIgnoreWhitespace(bool spaces = true, bool tabs = true, bool newLines = false) {
			LinkedListNode<string> node = m_node;
			while(node != null) {
				string str = node.Value;
				if((spaces && str == " ") || (tabs && str == "\t") || (newLines && str == "\n")) {
					node = node.Next;
					continue;
				}
				break;
		...
		public int SkipWhitespace(bool spaces = true, bool tabs = true, bool newLines = false) {
			int skipped = 0;
			while(HasNext()) {
				string str = Peek();
				if((spaces && str == " ") || (tabs && str == "\t") || (newLines && str == "\n")) {
